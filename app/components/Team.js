const Team = () =>
{
    ...
    return(
        <>
            <section id="team" className="grid grid-cols-1 min-h-screen gap-8 mx-8 content-start max-xl:h-[110dvh] max-md:h-[140dvh]">
                ...
                <div className="flex flex-wrap justify-center gap-5 p-4">
                    {team.map((member) => (
                        <div key={member.id} className="w-full h-80 max-md:h-52 md:w-[calc(33.3%-1.5rem)] border-3 rounded-2xl shadow-[0_0_50px] shadow-gray-700 transition-all duration-700 hover:scale-104 p-3 flex flex-col gap-8 max-md:gap-2 max-xl:gap-1">
                            <h2 className="text-3xl text-center max-md:text-xl max-xl:text-2xl">{member.name}</h2>
